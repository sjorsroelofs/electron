  ...
  if (env->HasVar("NODE_OPTIONS")) {
    ...
    for (const auto& part : parts) {
      ...
      if (IsPackagedApp() &&
        ...
        LOG(WARNING) << "Most NODE_OPTIONs are not supported in packaged apps."
                     << " See documentation for more details.";
        new_options.erase(new_options.find(option), part.length());
        ...
        LOG(WARNING) << "The NODE_OPTION " << option
